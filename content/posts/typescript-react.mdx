...
- [TypeScript - podstawowe typy, funkcje, tablice i interfejsy](/typescript-podstawy)
- [TypeScript - Generics, klasy i zaawansowane typy](/typescript-sredniozaawansowany/).
...
Zamiast `type` możesz również używać `interfejsów`, wspominałem o ich różnicach w [poprzednim wpisie](typescript-sredniozaawansowany).
...
Taka konstrukcja może Ci się kojarzyć z [typami generycznymi](/typescript-sredniozaawansowany/#typy-generycznegenerics).
...
`Action` to tzw. [Discriminated Unions](/typescript-sredniozaawansowany/#discriminated-unions).
...
Z hooka zwracamy [union type](/typescript-sredniozaawansowany/#union-types-i-type-guard), co w naszym przypadku jest **niechcianym zachowaniem**.
Możemy to zmienić na dwa sposoby:

- Zastosować [const assertion](/typescript-podstawy/#const-assertions).
- Zwrócić wartość jako [tuple array](/typescript-podstawy/#tuple-array).
...
Typy generyczne wspierają [union types](/typescript-sredniozaawansowany/#union-types-i-type-guard), nic nie stoi na przeszkodzie, żebyśmy taki zdefiniowali:
...
Przeróbmy przykład z Reactowych docsów na TypeScripta. Wykorzystujemy tutaj [asercje typów](/typescript-podstawy/#asercja-typow), reszta nie powinna być dla Ciebie nowością.
...
Zamiast `action constants` znalazłem zastosowanie dla [enumów](/typescript-podstawy/#enum). Definiujemy tutaj enuma **UserTypes**, który będzie nam jeszcze potrzebny za chwilę, przy reducerach. Jest jeszcze interface **UserActionTypes** i alias **Name**. Całość spinamy w naszą akcję:
...
Całość wydaje się **mocno przytłaczająca** przez wszechobecne [typy generyczne](/typescript-sredniozaawansowany/#typy-generycznegenerics).
...
Podajemy tutaj typ propsa w [object type literal](/typescript-sredniozaawansowany/#type-vs-interface).
