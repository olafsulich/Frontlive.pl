...
Mimo tego że CSS nie jest językiem programowania, to często Frotnend Developerzy popełniają w nim te same błędy. Zebrałem je w całość i stworzyłem listę Antywzorców w CSS, czyli najczęściej popełnianych błędów początkujących i nie tylko 😉

## Stylowanie po id i tagach

Stylowanie po id i tagach to zdecydowanie najczęściej popełniany błąd 😱 Co w tym takiego strasznego? Pisanie w ten sposób znacząco **zwiększa specyficzność** przez co style stają się cięższe w utrzymaniu i ponownym reużywaniu.
...
Jaką wielkość będzie miał nasz tekst? Spodziwalibyśmy się, że ostatnia z deklaracji będzie tą właściwą, niestety wynik jest inny:
...
Poza psuciem specyficzności, stylowanie za pomocą kombinacji tagów `body div span` staje się coraz mniej czytalne, a gdy dodamy do tego id i klasy, to robi nam się małe nazewnicze piekiełko 🔥
...
Stylowanie po tagach **nie zawsze** jest złe. Istnieją tagi, dla których chcemy mieć konkretnie style, które nie będą się zmieniać. Przykładem tego może być `html`/`body` czy wykonywanie tzw. _normalize_.
...
Jak to naprawić? Ponownie, wystarczy dodać odpowienią klasę:
...
Do zarządzania `z-index` przydadzą nam się [zmienne CSS](https://frontlive.pl/blog/css-variables), to dzięki nim będzie mogli uporządkować i przede wszystkim odpowiednio nazwać poszczególne indexy. W `:root` deklarujemy zmienne z odpowienimi nazwami:
...
Trzymanie się pewnych założeń, konwencji w kodzie to bardzo ważna rzecz, nie tylko pod względem technologicznym, ale również zespołowym. Nie warto tracić czasu na sprzeczanie się, czy lepszą nazwą klasy jest `.box-text`, czy `.box_text`. Dzięki takiej pewnej niekonsekwncji my sami po czasie możemy się pogubić jak tak naprawdę powinniśmy pisać w naszym projekcie.
...
Podczas korzystania z flexboxa i grida mamy możliwość zmieniania kolejności elementów w kontenerze. O ile może być to czasem przydatne, to najczęściej nie jest to dobrą praktyką. Załóżmy, że mamy kontener, a w nim pięć elementów, jeśli w kontenerze ustawimy właściwość `flex-direction` na `row-reverse`, to elementy zostaną wyświetlone w odwrotnej kolejności. Zmienienie kolejności w CSS nie spowoduje jednak przemieszczenia się elementów w strukturze HTML, a ten potrzebny jest dla czytników ekranowych. Dla użytkowników czytników może być to dość frustrujące, gdy będą przemieszczali się po elementach w złej kolejności. Jeśli checesz zmieniać kolejność, zrób to w HTML:
...
Rozwiązaniem tego problemu jest ostylowanie ouline lub zapewnienie innych styli, które posłużą nam za taki wizualny wskaźnik. Pamiętaj, aby nie korzystać tutaj z samych kolorów, niektórzy użytkownicy mogą ich nie rozróżniać:
