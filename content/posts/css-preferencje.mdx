...
W `@prefers-reduced-motion` możemy rozróżnić dwa stany - `no-preference` i `reduce`. Załóżmy, że na naszej stronie chcemy mieć box, który będzie się obracał w konkrenty sposób przez półtorej sekundy:
...
Warto pamiętać, że `@prefers-reduced-motion` nie ograniczenia nas w żadnym stopniu. Nie musimy również kasować całej animacji, możemy ją tylko **zredukować**, o ile i w jaki sposób, to już zależy od konkrentego przypadku.
...
Którą strone obrać? _Jeśli mam wybierać między złem a złem..._ Najlepiej nie wybierać wcale i przygotować dwie wersje tak, aby każdy był zadwolony. Dzięki `@prefers-color-scheme` mamy możliwość wykrycia preferowanego motywu w systemie:
...
Preferencje kontrastu to nie tylko kolory! Zamiast zmieniać ich poziom, możemy dostować inne elemnty UI, taki jak np. _border_, aby były bardziej wyraziste:
...
Last but not least - czyli preferencje dotyczące transparentości. Dzięki `@prefers-reduced-transparency` możemy wskazać w jakim stopniu dany element UI będzie transparenty:
