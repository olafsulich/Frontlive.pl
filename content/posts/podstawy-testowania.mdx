...
Podczas tworzenia aplikacji bardzo często trafiamy na najróżniejsze błędy. Najlepiej by było łapać je **jak najwcześniej**, żeby przypadkowo nie wypłynęły na produkcję (pozdro mBank).

Oczywiście testy pomagają nam w tym. Dzięki nim tak naprawdę oszczędzamy masę czasu, który byśmy musieli poświęcić na debugowanie popsutego kodu.

Największą zaletą testów jest **pewność**, że nasz kod będzie działać tak, jak tego oczekujemy. Oczywiście nie mamy nigdy 100% pewności, że coś się nagle nie wysypię, ale otestowany kod to zawsze większa pewniejszy kod.
...
Wspomniane trofeum skupia się, w przeciwieństwie do piramidy, na testach **integracyjnych**. Przyjrzyjmy się trochę bliżej rodzają testów i ich rozłożeniu.
...
Kolejnym narzędziemy, a właściwie językiem, jest uwielbiany przeze mnie **TypeScript**. Pozwala Ci on na statyczne typowanie, które nie jest dostępne w czystym JS. To właśnie dzięki niemu oszczędamy sobie wielu godzin na debugowaniu głupich błędów. Założeniem TypeScripta jest to, o czym wcześniej wspominałem, czyli **pewność**, w tym przypadku typów.
...
Przykład technologii: [React Testing Library](https://testing-library.com/docs/react-testing-library/intr)
...
Jak może wyglądać taki test? Załóżmy, że mamy jakiś formularz logowania. W testach e2e nasz robot musi odpowiednio kliknąć w inputy i wprowadzić dane, a następnie zatwierdzić je przycisk _Zaloguj się_. Po zalogowaniu powinniśmy zobaczyć tekst _Zalogowano_.
...
To prawda, na samym końcu jest użytkownik, ale to nie znaczy, że powinniśmy pisać tylko i wyłącznie testy e2e.
...
Przez następne tygodnie, na tym blogu, pokażę Ci **jak testować kod** za pomocą wspomianych dziś rodzajów testów. Zaczniemy od Jest, następnie przejdziemy do React Testing Library, a skończymy na testach e2e w Cypressie.
