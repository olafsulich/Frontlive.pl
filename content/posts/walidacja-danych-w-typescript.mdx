...
excerpt: 'Jak zapewnić jeszcze większe bezpieczeństwo aplikacji stosując walidację w TypeScript?'
---
...
<Image src="/images/walidacja-danych-na-frontendzie/image-1.png" />
...
- **User Experience**
  ...
  Wyobraź sobie, że wypełniach naprawdę długi formularz... Wysyłasz i bum! Okazuję się, że w polu z adresem email brakuje `@`.
...
Zaczynijmy od zdeklarowania schematu danych. Możemy definiować typy proste, złożone, wskazywać opcjonalne pola, enumy, niestandardowe wiadomości błędów... Jest tego masa!
...
async function fetchPosts() {
  ...
  const parsedData = DataSchema.parse(data); // <- poprawny typ ✅
}
```
...
Natomiast, jeśli chcielibyśmy _wyciągnąć_ typ ze schematu, możemy to zrobić dzięki specjalnemu mechanizmowi infernecji `zod.infer`:
